...
function GerarMail() {
    ...
    xTextoEmail += "<tr style='height: 5px;'><td colspan='4'></td></tr>"
                ...
                + "<tr><td colspan='4' style='height:23px; padding-bottom:8px;'> Código de Barras: <font size=2pt><b>" + document.FormCriarMail.CodigoDeBarras.value + "</b></font></td></tr>"
                + "<tr><td colspan='4' style='border-top: 1px solid #D3D3D3;'></td></tr>"
...
function ExportaBancoDeDados() {
    ...
    localDB.transaction(function(transaction){
            transaction.executeSql(query, [], function(transaction, results){
           	...
           	for (var i = 0; i < results.rows.length; i++) {
                var row = results.rows.item(i);
                xDadosPropostas = xDadosPropostas +
                    row['Id'] + ";" +
                    row['NomeUsuario'] + ";" +
                    row['TelefoneUsuario'] + ";" +
                    row['uVeiculo'] + ";" +
            ...
            document.FormConfigurar.ImportExport.value = xDadosPropostas;
        }, function(transaction, error){return;})
    });
    ...
    setTimeout(function(){
        ...
        const content = document.getElementById("ImportExport").value;
        const file = new Blob([content], { type: 'text/plain;charset=utf-8' });
        link.href = URL.createObjectURL(file);
        link.download = "RciTecPropostas_Bkp.txt";
        link.click();
...
function ImportaBancoDeDados() {
    ...
    input.addEventListener("change", function() {
        if (this.files && this.files[0]) {
            ...
            localDB.transaction(function(tx) {tx.executeSql('DROP TABLE Veiculos;');});
            localDB.transaction(function(tx) {tx.executeSql('DROP TABLE Propostas;');});
            setTimeout(function(){localDB.transaction(function(tx) {tx.executeSql('CREATE TABLE IF NOT EXISTS User(Id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, NomeUsuario TEXT NULL, TelefoneUsuario TEXT NULL, uVeiculo INTEGER NULL, Auth TEXT NULL);');});},500);
            setTimeout(function(){localDB.transaction(function(tx) {tx.executeSql('CREATE TABLE IF NOT EXISTS Veiculos(Id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, Veiculo TEXT NULL, ValorCombustivel INTEGER NULL, MediaConsumo INTEGER NULL, PercentualMaximoKm INTEGER NULL, PercentualMinimoKm INTEGER NULL);');});},1000);
            setTimeout(function(){localDB.transaction(function(tx) {tx.executeSql('CREATE TABLE IF NOT EXISTS Propostas(Id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, Origem TEXT NULL, Destino TEXT NULL, Nome TEXT NULL, Telefone TEXT NULL, IdVeiculo INTEGER NULL, Veiculo TEXT NULL, ValorLitroCombustivel INTEGER NULL, Distancia INTEGER NULL, Pedagio INTEGER NULL, Ajudante INTEGER NULL, Outros INTEGER NULL, PorcentagemDesconto TEXT NULL, Ajuste INTEGER NULL, MediaConsumo INTEGER NULL, PercentualMaximoKm INTEGER NULL, PercentualMinimoKm INTEGER NULL, DescontoFreteLiquido INTEGER NULL, TotalDosServicos INTEGER NULL, DataRegistro INTEGER NULL, Busca TEXT NULL, Email INTEGER FALSE);');});},1500);
...
function ConverteDados() {
    var ImportExport = document.querySelector('#ImportExport').value;
    var xCampo = 1;
    var xLinha1 = 1;
    var xLinha2 = 1;
    var xLinha3 = 1;
    ...
    for (var i = 0; i < ImportExport.length; i++) {
        if (ImportExport.substr(i,1)=="@"){i++; xBase++; xCampo=1};
        if (ImportExport.substr(i,1)!=";"){xCaracter=xCaracter+ImportExport.substr(i,1);};

        if (ImportExport.substr(i,1)==";" && xBase==1){
            if (xCampo==2){window['xNomeUsuario'+xLinha1]=xCaracter;};
            if (xCampo==3){window['xTelefoneUsuario'+xLinha1]=xCaracter;};
            if (xCampo==4){window['xuVeiculo'+xLinha1]=xCaracter;};
            if (xCampo==5){window['xAuth'+xLinha1]=xCaracter; xCampo=0; xLinha1++;};
            xCaracter = "";
            xCampo++;
        };

        if (ImportExport.substr(i,1)==";" && xBase==2){
            if (xCampo==2){window['xVeiculo1'+xLinha2]=xCaracter;};
            if (xCampo==3){window['xValorCombustivel'+xLinha2]=xCaracter;};
            if (xCampo==4){window['xMediaConsumo1'+xLinha2]=xCaracter;};
            if (xCampo==5){window['xPercentualMaximoKm1'+xLinha2]=xCaracter;};
            if (xCampo==6){window['xPercentualMinimoKm1'+xLinha2]=xCaracter; xCampo=0; xLinha2++;};
            xCaracter = "";
            xCampo++;
        };

        if (ImportExport.substr(i,1)==";" && xBase==3){
            if (xCampo==2){window['xOrigem'+xLinha3]=xCaracter;};
            if (xCampo==3){window['xDestino'+xLinha3]=xCaracter;};
            if (xCampo==4){window['xNome'+xLinha3]=xCaracter;};
            if (xCampo==5){window['xTelefone'+xLinha3]=xCaracter;};
            if (xCampo==6){window['xIdVeiculo'+xLinha3]=xCaracter;};
            if (xCampo==7){window['xVeiculo2'+xLinha3]=xCaracter;};
            if (xCampo==8){window['xValorLitroCombustivel'+xLinha3]=xCaracter;};
            if (xCampo==9){window['xDistancia'+xLinha3]=xCaracter;};
            if (xCampo==10){window['xPedagio'+xLinha3]=xCaracter;};
            if (xCampo==11){window['xAjudante'+xLinha3]=xCaracter;};
            if (xCampo==12){window['xOutros'+xLinha3]=xCaracter;};
            if (xCampo==13){window['xPorcentagemDesconto'+xLinha3]=xCaracter;};
            if (xCampo==14){window['xAjuste'+xLinha3]=xCaracter;};
            if (xCampo==15){window['xMediaConsumo2'+xLinha3]=xCaracter;};
            if (xCampo==16){window['xPercentualMaximoKm2'+xLinha3]=xCaracter;};
            if (xCampo==17){window['xPercentualMinimoKm2'+xLinha3]=xCaracter;};
            if (xCampo==18){window['xDescontoFreteLiquido'+xLinha3]=xCaracter;};
            if (xCampo==19){window['xTotalDosServicos'+xLinha3]=xCaracter;};
            if (xCampo==20){window['xDataRegistro'+xLinha3]=xCaracter;};
            if (xCampo==21){window['xBusca'+xLinha3]=xCaracter;};
            if (xCampo==22){window['xEmail'+xLinha3]=xCaracter; xCampo=0; xLinha3++;};
            xCaracter = "";
            xCampo++;
        };
    };

    for (let i=1; i<xLinha1; i++) {
        localDB.transaction(function(tx) {tx.executeSql('INSERT INTO User (NomeUsuario, TelefoneUsuario, uVeiculo, Auth) VALUES ("'+window['xNomeUsuario'+i]+'", "'+window['xTelefoneUsuario'+i]+'", '+window['xuVeiculo'+i]+', "'+window['xAuth'+i]+ '");');});
    };
