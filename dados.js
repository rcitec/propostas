	...
	localDB.transaction(function(transaction){
			...
			var tabela = "<hr>" +
            	...
            	for (var i = 0; i < results.rows.length; i++) {
                    ...
                    xResultado = (xData.substr(0,17) + " | " + row['Origem'] + " X " + row['Destino'] + " | " + row['Nome']).substr(0,63);
...
function GravarConfigApp() {
    ...
    // Atualiza Combustivel, Média Consumo e Desconto Por FaixaKm
    xValorCombustivel = Number(document.FormApp.ValorCombustivel.value);
    localDB.transaction(function(tx) {tx.executeSql('UPDATE Combustivel SET ValorCombustivel='+xValorCombustivel+', MediaConsumo='+xMediaConsumo+', DescontoPorFaixaKm='+xDescontoPorFaixaKm+', NomeUsuario="'+xNomeUsuario+'", TelefoneUsuario="'+xTelefoneUsuario+'";');});
